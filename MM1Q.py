...
def QSim(lambd=False, mu=False, simulation_time=False):
    ...
    if not lambd:
        lambd = eval(input("Inter arrival rate: "))
    if not mu:
        mu = eval(input("Service rate: "))
    if not simulation_time:
        simulation_time = eval(input("Total simulation time: "))
    ...
    if eval(input("Output data to csv (True/False)? ")):
        outfile = open(
            "MM1Q-output-(%s,%s,%s).csv" % (lambd, mu, simulation_time), "wb"
        )
